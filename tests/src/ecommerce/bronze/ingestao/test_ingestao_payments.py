...
from src.ecommerce.bronze.ingestao.ingestao_payments import BronzeIngestion


def test_ingestao_payments_inicializa_spark_e_salva():
    spark_session.reset_mock()
    session = MagicMock()
    spark_session.builder.appName.return_value.getOrCreate.return_value = session

    ingestao = BronzeIngestion(app_name="teste-payments")

    arquivo = "payments.csv"
    tabela = "workspace.bronze_payments"
    df = MagicMock()
