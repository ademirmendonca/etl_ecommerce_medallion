...
for name in ["StructType", "StructField", "StringType", "IntegerType", "DateType", "TimestampType"]:
...
def test_ingestao_reviews_inicializa_spark_e_salva():
    spark_session.reset_mock()
    session = MagicMock()
    spark_session.builder.appName.return_value.getOrCreate.return_value = session

    ingestao = BronzeIngestion(app_name="teste-reviews")

    arquivo = "reviews.csv"
    tabela = "workspace.bronze_reviews"
    df = MagicMock()
