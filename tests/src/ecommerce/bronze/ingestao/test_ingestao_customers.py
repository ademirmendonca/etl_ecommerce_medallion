...
# Mock do Spark para permitir testar o módulo sem pyspark instalado
...
def test_ingestao_customers_inicializa_spark_e_salva():
    spark_session.reset_mock()
    session = MagicMock()
    spark_session.builder.appName.return_value.getOrCreate.return_value = session

    ingestao = BronzeIngestion(app_name="teste-customers")

    arquivo = "clientes.csv"
    tabela = "workspace.bronze_customers"
    df = MagicMock()
