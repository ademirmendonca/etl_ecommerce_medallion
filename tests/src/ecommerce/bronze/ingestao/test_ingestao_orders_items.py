...
for name in ["StructType", "StructField", "StringType", "TimestampType", "IntegerType", "DecimalType"]:
...
from src.ecommerce.bronze.ingestao.ingestao_orders_items import BronzeIngestion


def test_ingestao_orders_items_inicializa_spark_e_salva():
    spark_session.reset_mock()
    session = MagicMock()
    spark_session.builder.appName.return_value.getOrCreate.return_value = session

    ingestao = BronzeIngestion(app_name="teste-orders-items")

    arquivo = "orders_items.csv"
    tabela = "workspace.bronze_orders_items"
    df = MagicMock()
